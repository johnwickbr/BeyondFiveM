...
local cabs = {
   { hash=0xC703DB5F, x=320.167, y=3407.892, z=36.341, a=256.213 },
   { hash=0xC703DB5F, x=320.904, y=3410.567, z=36.322, a=256.213 },
   { hash=0xC703DB5F, x=321.905, y=3413.080, z=36.298, a=256.213 },
   { hash=0xC703DB5F, x=322.304, y=3415.749, z=36.279, a=256.213 },
   { hash=0xC703DB5F, x=322.729, y=3418.390, z=36.248, a=256.213 },
   { hash=0xC703DB5F, x=323.296, y=3420.875, z=36.213, a=256.213 },
  }
...
downtownc = AddBlipForCoord(346.200, 3406.9194335938, 36.536968231201)
...
function StartJob(jobid)
    ...
    jobs.coords.cx[92],jobs.coords.cy[92],jobs.coords.cz[92] = -798.090, 831.699, 204.351
		...
		Wait(500)
		TriggerEvent("chatMessage", "", {0,0,0}, "Drive around and look for ^4clients.")
		onJob = jobid
	end
end
				...
				if IsPedSittingInVehicle(GetPlayerPed(-1), jobs.cars[1]) then
					if DoesEntityExist(jobs.peds[1]) then
						if IsPedFatallyInjured(jobs.peds[1]) then
							...
							TriggerEvent("chatMessage", "", {0,0,0}, "Your ^4client ^7has left. Find a new one.")
						else
							if jobs.flag[1] == 1 and jobs.flag[2] > 0 then
								...
								if jobs.flag[2] == 0 then
									...
									Citizen.InvokeNative(0xB736A491E64A32CF,Citizen.PointerValueIntInitialized(jobs.peds[1]))
									jobs.peds[1] = nil
									TriggerEvent("chatMessage", "", {0,0,0}, "Your ^4client ^7got tired of waiting. Find a new one.")
									jobs.flag[1] = 0
									jobs.flag[2] = 59+GetRandomIntInRange(1, 61)
								else
									if IsPedSittingInVehicle(GetPlayerPed(-1), jobs.cars[1]) then
							...
							if jobs.flag[1] == 2 and jobs.flag[2] > 0 then
								...
								if jobs.flag[2] == 0 then
									...
									Citizen.InvokeNative(0xB736A491E64A32CF,Citizen.PointerValueIntInitialized(jobs.peds[1]))
									jobs.peds[1] = nil
									TriggerEvent("chatMessage", "", {0,0,0}, "Your ^4client ^7does not want to go with you. Find a new ^4client.")
									jobs.flag[1] = 0
									jobs.flag[2] = 59+GetRandomIntInRange(1, 61)
								else
									if IsPedSittingInVehicle(jobs.peds[1], jobs.cars[1]) then
										...
										if street[2] ~= 0 and street[2] ~= nil then
											local streetname = string.format("Drive your ^4client^7 to ^3%s, %s", GetStreetNameFromHashKey(street[1]),GetStreetNameFromHashKey(street[2]))
                      TriggerEvent("chatMessage", "", {255,0,0}, streetname)
										else
											local streetname = string.format("Drive your ^4client^7 to ^3%s", GetStreetNameFromHashKey(street[1]))
                      TriggerEvent("chatMessage", "", {255,0,0}, streetname)
										end
                  ...
                  TriggerEvent("chatMessage", "", {0,0,0}, "You have delievered your ^4client ^7to the destination.")
									-- pay money on something
									Wait(8000)
									TriggerEvent("chatMessage", "", {0,0,0}, "Drive around and look for a new ^4client^7.")
									jobs.flag[1] = 0
									jobs.flag[2] = 59+GetRandomIntInRange(1, 90)
								end
							end
						...
						if jobs.flag[1] > 0 then
							jobs.flag[1] = 0
							jobs.flag[2] = 59+GetRandomIntInRange(1, 61)
							TriggerEvent("chatMessage", "", {0,0,0}, "Drive around and look for ^4clients^7.")
							if jobs.blip[1] ~= nil and DoesBlipExist(jobs.blip[1]) then
								Citizen.InvokeNative(0x86A652570E5F25DD,Citizen.PointerValueIntInitialized(jobs.blip[1]))
								jobs.blip[1] = nil
							end
						...
						if jobs.flag[1] == 0 and jobs.flag[2] > 0 then
							...
							if jobs.flag[2] == 0 then
								...
								if DoesEntityExist(rped) then
									...
									TriggerEvent("chatMessage", "", {0,0,0}, "Your ^4client ^7is waiting for you. Drive closer.")
					...
					if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)), GetEntityCoords(jobs.cars[1]), true) > 30.0001 then
						...
						TriggerEvent("chatMessage", "", {0,0,0}, "Get back in your cab to continue working or leave to ^1quit^7.")
            wait(20000)
					end
				end
			else
				StopJob(1)
				TriggerEvent("chatMessage", "", {0,0,0}, "Your taxi was ^1destroyed^7.")
			end
		end
	end
end)
